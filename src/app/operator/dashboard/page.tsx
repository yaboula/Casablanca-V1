  ...
  return (
    <div className="max-w-lg mx-auto">
      ...
      <div className="px-5 pt-6 pb-4">
        <p className="text-sm text-slate-400 capitalize">{today} · CMN Operations</p>
            ...
            >
              ...
              <div className="flex gap-2">
                <Link
                  ...
                  className="flex-1 min-h-[44px] bg-brand-primary/15 text-brand-primary font-semibold text-xs rounded-xl
                             flex items-center justify-center gap-1.5 hover:bg-brand-primary/25 transition-colors"
                >
                ...
                <Link
                  href={`/operator/delivery/${delivery.id}`}
                  className="flex-1 min-h-[44px] bg-slate-800 text-slate-300 font-semibold text-xs rounded-xl
                             flex items-center justify-center gap-1.5 hover:bg-slate-700 transition-colors"
                >
...
function StatCard({
  ...
  return (
    <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4">
      ...
      <p className={`text-2xl font-black mt-1 tabular-nums ${accent ? "text-amber-400" : "text-white"}`}>
        {value}
      </p>
    </div>
  );
...
function UrgencyBadge({ level, label }: { level: string; label: string }) {
  if (level === "CRITICAL") {
    return (
      <motion.span
        ...
        className="inline-flex items-center gap-1 bg-red-500/20 text-red-400 text-[10px] font-bold uppercase tracking-wider px-2 py-1 rounded-full"
      >
  ...
  if (level === "SOON") {
    return (
      <motion.span
        ...
        className="inline-flex items-center gap-1 bg-amber-500/20 text-amber-400 text-[10px] font-bold uppercase tracking-wider px-2 py-1 rounded-full"
      >
  ...
  return (
    <span className="inline-flex items-center gap-1 bg-slate-700 text-slate-300 text-[10px] font-bold uppercase tracking-wider px-2 py-1 rounded-full">
