...
import { CheckCircle2, X, FileText, CreditCard } from "lucide-react";
...
export default function DocumentReviewList({ documents }: Props) {
  const [showRejectModal, setShowRejectModal] = useState(false);
  const [customReason, setCustomReason] = useState("");
  const [busy, setBusy] = useState(false);

  // Always show the first document in the queue
  const currentDoc = documents[0];

  const handleApprove = useCallback(async () => {
    if (!currentDoc || busy) return;
    setBusy(true);
    try {
      await approveDocument(currentDoc.id);
      toast.success(
        `${currentDoc.customerName} — ${
          currentDoc.type === "PASSPORT" ? "Pasaporte" : "Carnet"
        } aprobado`
      );
    } catch {
  ...
  const handleReject = useCallback(
    async (reason: string) => {
      ...
      setBusy(true);
      try {
        await rejectDocument(currentDoc.id, reason);
  ...
  const remaining = documents.length;
        ...
        {showRejectModal && (
          <>
            ...
            >
              ...
              <h3 className="text-base font-bold text-slate-900 mb-4">Motivo del rechazo</h3>
                ...
                {REJECTION_REASONS.map((reason) => (
                  <button
                    ...
                    className="w-full min-h-[48px] text-left px-4 py-3 bg-slate-50 hover:bg-slate-100 border border-slate-200
                               rounded-xl text-sm text-slate-900 font-medium transition-colors disabled:opacity-50"
                  >
                    {reason}
                  </button>
                ))}
              ...
              <div className="flex gap-2">
                ...
                <button
                  ...
                  disabled={!customReason.trim() || busy}
...
function DocumentReviewCard({ doc, onDragEnd, onApprove, onReject, busy }: DocCardProps) {
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 bg-slate-100 flex items-center justify-center relative">
        {doc.fileUrl && doc.fileUrl !== "/images/vehicles/placeholder.svg" ? (
      ...
      <div className="p-4 space-y-3">
        ...
        <div className="space-y-1.5">
          ...
          {doc.uploadedAgo && <InfoRow label="Subido hace" value={doc.uploadedAgo} />}
        </div>
        ...
        <div className="flex gap-2 pt-2">
          <button
            ...
            className="flex-1 min-h-[48px] bg-red-50 text-red-600 font-bold text-sm rounded-xl
                       flex items-center justify-center gap-1.5 hover:bg-red-100 active:scale-[0.98]
          ...
          <button
            ...
            className="flex-1 min-h-[48px] bg-emerald-50 text-emerald-600 font-bold text-sm rounded-xl
                       flex items-center justify-center gap-1.5 hover:bg-emerald-100 active:scale-[0.98]
                       disabled:opacity-50 transition-all"
          >
